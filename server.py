...
def ensure_ca_bundle():
    """CA 证书包自愈守卫（best-effort，非致命）。

    目的：托管 Python 的 certifi cacert.pem 一旦缺失/损坏，东财等 HTTPS 数据源会
    报 SSL 证书校验错误。此处启动时校验，若不可用则用本级 python -m pip 就地重装，
    并把 SSL_CERT_FILE / REQUESTS_CA_BUNDLE / CURL_CA_BUNDLE 指到该 bundle，
    让 requests / httpx / aiohttp / urllib3 统一走同一份证书。

    关键约束：
    - 必须在 import config / fastapi / services 之前运行，否则 requests 可能在
      导入时缓存默认 CA 上下文，env 设置失效。
    - 全程 try/except + 明确日志，绝不抛异常中断启动（数据源有本地降级链兜底）。
    """
    try:
        import certifi  # noqa: F401
        ca_path = certifi.where()
        if ca_path and os.path.exists(ca_path) and os.path.getsize(ca_path) > 0:
            os.environ.setdefault("SSL_CERT_FILE", ca_path)
            os.environ.setdefault("REQUESTS_CA_BUNDLE", ca_path)
            os.environ.setdefault("CURL_CA_BUNDLE", ca_path)
            LOG.info("[CA] certifi bundle OK: %s", ca_path)
            return
        LOG.warning("[CA] certifi.where() 返回无效路径，尝试重新安装: %r", ca_path)
    except Exception as e:
        LOG.warning("[CA] certifi 导入失败，尝试重新安装: %s", e)
    # 自愈：用托管 Python 就地重装 certifi
    try:
        import subprocess
        LOG.info("[CA] 正在 pip 安装 certifi ...")
        subprocess.run([sys.executable, "-m", "pip", "install",
                        "--quiet", "--disable-pip-version-check", "certifi"],
                       check=False, timeout=180)
        import certifi as c2
        ca_path = c2.where()
        if ca_path and os.path.exists(ca_path) and os.path.getsize(ca_path) > 0:
            os.environ["SSL_CERT_FILE"] = ca_path
            os.environ["REQUESTS_CA_BUNDLE"] = ca_path
            os.environ["CURL_CA_BUNDLE"] = ca_path
            LOG.info("[CA] certifi 自愈安装成功: %s", ca_path)
        else:
            LOG.warning("[CA] certifi 安装后仍无效，HTTPS 可能失败（数据源降级链兜底）")
    except Exception as e:
        LOG.warning("[CA] certifi 自愈失败（非致命，服务继续启动）: %s", e)
