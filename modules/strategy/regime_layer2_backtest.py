...
_UNIV_CACHE = {}
_UNIV_META = {}   # include_bj -> (tdx 数据末根日期, 构建时间)，用于缓存失效


def _tdx_data_version():
    """数据版本 = 上证指数 .day 末根日期（YYYYMMDD int）。数据更新后版本变化 → 缓存失效。"""
    try:
        import struct
        p = os.path.join("D:/new_tdx64/vipdoc/sh/lday/sh000001.day")
        with open(p, "rb") as f:
            f.seek(-32, 2)
            return struct.unpack("<iiiiifii", f.read(32))[0]
    except Exception:
        return None


def _get_universe(include_bj=True):
    """构建 Layer2 宇宙（进程内缓存，按 TDX 数据末根日期自动失效）。
    同一数据版本下重复调用秒级返回（Web 改持仓/改设置后重跑 analyze 不再全量构建）。"""
    ver = _tdx_data_version()
    meta = _UNIV_META.get(include_bj)
    if include_bj in _UNIV_CACHE and meta and meta[0] == ver:
