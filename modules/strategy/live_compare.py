...
注意：本脚本所在目录为 modules/strategy/（regime 引擎 WORK 目录，存放输入数据），
但前端看板 build_dashboard 从项目根 data/ 读取产物，因此本脚本把 live_buy_list 与
live_compare.json 统一写到 data/。

用法: python live_compare.py [cash] [N] [scheme]
"""
import os, sys, json

# 同目录导入 regime 引擎（current_candidates / format_live_report / _get_universe）
