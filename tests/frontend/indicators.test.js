// -*- coding: utf-8 -*-
// 技术指标算法单元测试（从 deepthinkSingle 继承，文档承诺的"指标算法 5 例"）
// 目的：防止 D1 类"算法改坏但无测试"回归——MACD 的 DEA/柱必须真实渲染。
// 运行: node --test --test-force-exit tests/frontend/indicators.test.js
const test = require('node:test');
const assert = require('node:assert');
const path = require('path');

// indicators.js 通过 globalThis.DTIndicators 暴露（Node 下 window 未定义）
require(path.join(__dirname, '..', '..', 'static', 'js', 'charts', 'indicators.js'));
